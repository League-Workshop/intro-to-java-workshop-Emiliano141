...
walle.microMove(100);
	...
	private void spaceBarWasPressed() {
		...
		// 5.2 Change ROBOTNAME below to match the name of the robot you created in step 1.  THEN, remove the slashes at the beginning of the next two lines
		...
		System.out.println("Y:"+ robotYLocation);
		// 7. If robot is at same location as the little girl
		//      --make a pop-up tell the robot where to go next
		if (robotXLocation==750 && robotYLocation==400)
		{
		
		
		JOptionPane.showMessageDialog(null, "The next clue is with the boy in the blue cap");
		}
		// 8. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)
		if (robotXLocation==250 && robotYLocation==400)
		{
			JOptionPane.showMessageDialog(null, "Go to where the birds rest");
		}
		// 9.  If the robot is in the final location
		//     --call the treasureFound() method
		if(robotXLocation==650 && robotYLocation==100) {
